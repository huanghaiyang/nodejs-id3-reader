...
ID3v2Frame['APIC'] = function readPictureFrame(offset, length, data, flags, v) {
    v = v || '3';
...
ID3v2Frame['COMM'] = function readCommentsFrame(offset, length, data) {
    var start = offset;
    var charset = getTextEncoding(data.getByteAt(offset));
...
ID3v2Frame['COM'] = ID3v2Frame['COMM'];

ID3v2Frame['PIC'] = function(offset, length, data, flags) {
    return ID3v2Frame['APIC'](offset, length, data, flags, '2');
};

ID3v2Frame['PCNT'] = function readCounterFrame(offset, length, data) {
    // FIXME: implement the rest of the spec
    return data.getInteger32At(offset);
};

ID3v2Frame['CNT'] = ID3v2Frame['PCNT'];

ID3v2Frame['T*'] = function readTextFrame(offset, length, data) {
    var charset = getTextEncoding(data.getByteAt(offset));

    return data.getStringWithCharsetAt(offset + 1, length - 1, charset).toString();
};

ID3v2Frame['TCON'] = function readGenreFrame(offset, length, data) {
    var text = ID3v2Frame['T*'].apply(this, arguments);
    return text.replace(/^\(\d+\)/, '');
};

ID3v2Frame['TCO'] = ID3v2Frame['TCON'];

//ID3v2Frame['TLEN'] = function readLengthFrame(offset, length, data) {
//    var text = ID3v2Frame['T*'].apply( this, arguments );
//    
//    return {
//        text : text,
...
ID3v2Frame['USLT'] = function readLyricsFrame(offset, length, data) {
    var start = offset;
    var charset = getTextEncoding(data.getByteAt(offset));
    var language = data.getStringAt(offset + 1, 3);
...
ID3v2Frame['ULT'] = ID3v2Frame['USLT'];
